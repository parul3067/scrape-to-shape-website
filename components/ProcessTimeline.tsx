...
export default function ProcessTimeline() {
  return (
    <section className="h-[100dvh] relative overflow-hidden py-10 px-4 sm:px-6 lg:px-8 bg-[#1C3A2F]">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="hidden lg:grid lg:grid-cols-3 gap-px bg-white/10 rounded-2xl overflow-hidden">
          {STEPS.map((step, index) => (
            <ScrollReveal
              key={step.num}
              delay={index * 0.08}
            >
              <div className="bg-[#1C3A2F] p-8 h-full flex flex-col gap-4">
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-full border border-[#C9A84C]/40 flex items-center justify-center text-[#C9A84C] shrink-0">
                    {step.icon}
                  </div>
                  <span className="text-[#C9A84C]/40 font-mono text-xs font-semibold tracking-wider">
        ...
        <div className="lg:hidden flex flex-col gap-0">
          <div className="relative">
            <div
              className="absolute left-[1.125rem] top-0 bottom-0 w-px bg-[#C9A84C]/20"
              aria-hidden="true"
            />
            {STEPS.map((step, index) => (
              <ScrollReveal
              ...
              >
                <div className="w-9 h-9 shrink-0 rounded-full border border-[#C9A84C]/40 flex items-center justify-center text-[#C9A84C] relative z-10 bg-[#1C3A2F]">
                  {step.icon}
                </div>
                <div className="pt-1">
                  <p className="text-[#C9A84C]/50 font-mono text-xs font-semibold tracking-wider mb-1">
                    {String(step.num).padStart(2, "0")}
                  </p>
